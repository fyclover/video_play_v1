  ...
  <div class='page-content'>
    ...
    <tao-table border style="margin-top: 15px" :data="roleList" :showPage="false">
      ...
      <el-table-column label="分类名" prop="title"/>
        ...
        <el-table-column label="是否是大视频" prop="status">
        <template slot-scope="scope">
          <el-tag size="mini" :type="scope.row.is_size ? '' : 'info'">
            {{scope.row.is_size == 1 ? '是' : '否'}}
          </el-tag>
          <el-switch   v-model="scope.row.is_size" @change="changes(scope.row.is_size,scope.row,'size')" :active-value="1" :inactive-value="0"></el-switch>
        </template>
      </el-table-column>
...
<script>
  import {getVideoTypeIsSizeApi,getVideoTypeApi,getVideoTypeListApi, getVideoTypeDelApi,getVideoTypeEditApi,getVideoTypeAddApi,getVideoTypeStatusApi,getVideoTypeIsShowApi} from '@/api/VideoTypeApi'
    ...
    methods: {
      ...
      getUserList(){
       ...
       uploadFile:function(file){
         ...
         fm.append('agent_uid', localStorage.getItem('admin_user_id'));
