...
<template>
  <div class='page-content'>
    <el-row :gutter="20">
      ...
      <el-col :xs="24" :sm="12" :lg="4" v-if="admin_type == 1">
      <el-select v-model="searchParameter.status" style="width:100%" placeholder="通知类型">
    ...
    <el-table style="margin-top: 15px" border :data="roleList" :showPage="false">
      ...
      <el-table-column label="视频热度" width="50" prop="heat"/>

      <el-table-column label="视频打赏" width="50" prop="video_money"/>
     ...
     <el-table-column label="审核状态" prop="status" v-if="admin_type == 1">
       <template slot-scope="scope">
      ...
      <el-table-column label="审核状态" prop="status"  v-if="admin_type == 2">
        <template slot-scope="scope">
            {{scope.row.status == 1 ? '审核通过' : '审核中'}}
        </template>
      </el-table-column>
      ...
      <el-form ref="form" :model="form" label-width="60px">
       <el-row style="display: flex">
        ...
        <el-row style="display: flex">
          ...
          <el-input type="number" v-model="form.video_price" placeholder="单片价格" style="width: 150px;"/>
        ...
        <el-select v-model="form.platform" multiple placeholder="请选择服务平台" style="width: 150px; margin-left: 10px">
          <el-option
       ...
       <el-row style="display: flex">
            <el-input v-model="form.labels" placeholder="输入标签#国风#古风以#号区分" style="flex: 1"/>
        </el-row>
        <el-row style="display: flex">
            <el-input v-model="form.duration" placeholder="免费时长" style="flex: 1"/>
        </el-row>
...
<script>
  var i=0;
  import {getVideoListApi,getVideoListStatusApi, getVideoDelApi,getVideoEditApi,getVideoAddApi,getVideoAuthApi,getVideoPlatformApi} from '@/api/VideoApi'
  ...
  export default {
    ...
    mounted() {
     ...
     this.admin_user_id = localStorage.getItem('admin_user_id');

      this.getUserList()
      this.getVideoPlatformApi()
    },
    methods: {
       ...
       uploadFile:function(file){
         ...
         axios.post('/upload/video',fm).then(res=>{
          if(res.code ==0){
           ...
           this.form.video_url = res.data.data[0]
      ...
      uploadFiles:function(file){
         var fm = new FormData();
         fm.append('file', file.file);
         fm.append('agent_uid', this.admin_user_id);
         axios.post(this.uploadImage,fm).then(res=>{
           var arr = {
               'url': res.data.data[0],
               'uid': file.file.uid,
           };
