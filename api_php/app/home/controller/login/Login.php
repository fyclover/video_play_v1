...
class Login extends BaseController
{
    ...
    public function register_test()
    {
        ...
        $this->registerEncourage($data['agent_id_1'],$this->model->id);
        $data['token'] = $token = home_api_token($this->model->id);
        $data['id'] = $this->model->id;

        //查询是否存在这条token的用户
        $update = (new HomeTokenModel())->where('user_id', $this->model->id)->update(['token' => $token, 'create_time' => date('Y-m-d H:i:s')]);

        //数据不存在时插入
        if ($update == 0) {
            (new HomeTokenModel)->insert(['token' => $token, 'user_id' => $this->model->id, 'create_time' => date('Y-m-d H:i:s')]);
        }
        //登陆成功后存入 session
        session('home_user', $data);
        (new \app\common\service\LoginLog())->login(2);//登陆日志
        if (empty($data)) return show([], config('ToConfig.http_code.error'), '成功');
        return show($data);
    }
    ...
    //注册代理获得奖励
    protected function registerEncourage($AgentId,$uid)
    {
        $register_encourage_type =   getConfigOne('register_encourage_type');
        $EncourageType = isset($register_encourage_type['value']) ? $register_encourage_type['value']: 1;
        ...
        //奖励 1 送3天会员
        if ($EncourageType == 1) {
            $period = new VideoUserSetValidityPeriod();
            $find  = $period->where('uid',$AgentId)->find();
            if (empty($find)){
                $period->insertGetId(['uid'=>$AgentId,'end_time'=>date('Y-m-d H:i:s',strtotime("$date +3 day"))]);
            }else{
                $date = $find['end_time'];
