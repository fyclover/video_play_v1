		...
		methods: {
			...
			getUserConfig() {
				...
				userService.getUserConfig({name: 'array_banner'}).then(res => {
					...
					// localStorage.setItem('array_banner_quanbu',res.data.quanbu);
					// localStorage.setItem('array_banner_fenlei',res.data.fenlei);
				}).catch(err => {
					console.log('err:',err)
				})
			},
			 ...
			 * **/
			getVideotypeList() {
				videoService.getVideotypeList({limit: 10}).then(res => {
					if(res.code != 1) {
						uni.showToast({
